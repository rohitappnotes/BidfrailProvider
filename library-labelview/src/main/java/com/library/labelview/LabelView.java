...
public class LabelView extends View {
    ...
    /** Determine the View width size */
    private int measureWidth(int widthMeasureSpec) {
        int result;
        int specMode = MeasureSpec.getMode(widthMeasureSpec);
        int specSize = MeasureSpec.getSize(widthMeasureSpec);
        if (specMode == MeasureSpec.EXACTLY) {//Size is determined to use directly
            result = specSize;
        } else {
            ...
            //If the measurement requirement of the parent view is AT_MOST, that is, a maximum value is limited, then a smaller value is taken from the system recommended value and the calculated value
